...
calcSigma <- function(X1,X2,l=1) {
  ...
  for (i in 1:nrow(Sigma)) {
    for (j in 1:ncol(Sigma)) {
      Sigma[i,j] <- exp(-0.5*(abs(X1[i]-X2[j])/l)^2)  # here you need to substitute with the appropriate k(.,.)
    }
  }
  return(Sigma)
}
...
x.star <- seq(0.5, 10.5, by = 0.5) # infamous grid
x.star <- matrix(x.star, ncol=1)
# Calculate the covariance matrix
sigma <- calcSigma(x.star,x.star)

# Generate a number of functions from the process
n.samples <- length(x.star)#n.samples samples from the GP
...
# Plot the result - n.samples trajectories
...
x <- f$x # "observed" values
#x.star <- matrix(seq(min(X)-1, max(X)+1, length=n.samples), ncol=1)
k.xx <- calcSigma(x,x)
...
# in the book. - predictive is a Gaussian with these two quantities
...
n.samples <- 50 # 50 samples for each component of the GP

values <- matrix(rep(0,length(x.star)*n.samples), ncol=n.samples)
for (i in 1:n.samples) {
  values[,i] <- mvrnorm(1, f.star.bar, cov.f.star) # 50 samples for each mvrnormal
}
values <- cbind(x=x.star,as.data.frame(values))
values <- melt(values,id="x")
...
df <- data.frame(cbind(x.star = x.star, f.star.bar=f.star.bar))
colnames(df) <- c("x.star", "f.star.bar")
fig2b <- ggplot(values,aes(x=x,y=value)) +
  geom_line(aes(group=variable), colour="grey80") +
  geom_line(data=df,aes(x=x.star,y=f.star.bar), colour="red", size=1) + 
...
df <- data.frame(cbind(f.bar.star, cov.f.star))
...
gg <- ggplot(values, aes(x=x,y=value)) + 
  geom_line(aes(group=variable), colour="grey80") +
  geom_line(data=df,aes(x=x.star,y=f.bar.star),colour="red", size=1) + 
...
# This code returns the predictive distribution.
